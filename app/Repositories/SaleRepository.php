...
class SaleRepository implements SaleRepositoryInterface
{
    ...
    public function getFilteredSales(array $filters, int $perPage = 15): LengthAwarePaginator
    {
        ...
        if (!empty($filters['from_date']) && !empty($filters['to_date'])) {
            $query->whereBetween('sale_date', [$filters['from_date'] . ' 00:00:00', $filters['to_date'] . ' 23:59:59']);
        }
    ...
    public function getSalesSummary(array $filters): array
    {
        ...
        if (!empty($saleIds)) {
            $totalItemsSold = SaleDetail::whereIn('sale_id', $saleIds)->sum('quantity') ?? 0;
    ...
    public function getSalesChartData(string $period, ?int $userId = null): array
    {
        ...
        if ($period === 'today') {
            ...
            for ($hour = 9; $hour <= 18; $hour++) {
                ...
                $hourQuery = Sale::whereDate('sale_date', now()->toDateString())
                    ->whereRaw('HOUR(sale_date) = ?', [$hour]);
            ...
            for ($i = 6; $i >= 0; $i--) {
                ...
                $dayQuery = Sale::whereDate('sale_date', $date->toDateString());
                if ($userId) {
            ...
            for ($i = 5; $i >= 0; $i--) {
                ...
                $monthQuery = Sale::whereRaw('MONTH(sale_date) = ?', [$date->month])
                    ->whereRaw('YEAR(sale_date) = ?', [$date->year]);
